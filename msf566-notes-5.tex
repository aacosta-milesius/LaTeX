...
Specifically, let $\epsilon_t$  denote the returns (or return residuals, net of a mean process) and assume that $\epsilon_t=\sigma_t z_t$, where $z_t\sim iid~ N(0,1)$  \index{iid -- independent and\\identically distributed} and where the series $\sigma_t^2$  are modeled by
\[
...
\ecaption{ARCH Test using R library, \texttt{FinTS}}\index{R language} 
\rpkg{FinTS}
\begin{verbatim}
data(m.intc7303)
...
\subsubsection{GARCH($m, s$) Model Specification}\label{garch-spec}
...
\ecaption{GARCH Model Fitting in R}\index{R language} 
\rpkg{fArma}\rpkg{fGarch}\rpkg{fSeries}
\begin{verbatim}
...
\]
where $\theta$ and $\gamma$ are real constants. Both $\epsilon_t$ and $|\epsilon_t|-E(g(|\epsilon_t|)$ are zero-mean ($E[g(\epsilon_t)]$) iid\index{iid -- independent and\\identically distributed} sequences with continuous distributions. A better way to see the asymmetry is
\[
g(\epsilon_t) =
	\begin{cases}
...
\label{eq:charma}
\end{equation}
where $\{\eta_t\}$ is a Gaussian (normally distributed) white noise series with mean zero and variance $\sigma^2_{\eta}, \{\delta_t\}=\{(\delta_{1t}, \ldots,a_{mt})'\}$ is a sequence of iid\index{iid -- independent and\\identically distributed} random vectors with mean zero and non-negative definite covariance matrix $\mathbf{\Omega}$, and $\{\delta_t\}$ is independent of $\{\eta_t\}$.
